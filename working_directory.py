...
# File suffixes that open in the editable text viewer.
TEXT_SUFFIXES = ('.in', '.pwi', '.out', '.dos', '.txt', '.cif', '.log')
...
def _classify_file(f):
    """Return a human-readable type label for a Quantum Espresso working file."""
    name = f.lower()
    if name.endswith('.cif'):
        return "Structure file"
    if name.endswith('.vasp') or f.endswith('POSCAR') or f.endswith('CONTCAR'):
        return "Structure file"
    if name.endswith('.upf'):
        return "Pseudopotential"
    if name.endswith('.in') or name.endswith('.pwi'):
        return "Input File - pw.x/pp.x parameters"
    if name.endswith('.xml'):
        return "Output File - Summary (QE XML)"
    if '.pdos' in name or name.endswith('.projwfc_up') or name.endswith('.projwfc_down'):
        return "Output File - Projected DOS"
    if name.endswith('.dos'):
        return "Output File - Density of States"
    if name.endswith('.cube'):
        return "Output File - Volumetric data (cube)"
    if name.endswith('.out') or name.endswith('.log'):
        return "Output File - Log"
    return "Other File"
...
def on_select_file(evt: gr.SelectData):
    ...
    if selected_file_name.endswith(STRUCTURE_SUFFIXES):
        selected_structure_file = selected_file_name
    else:
        selected_structure_file = None
    if selected_file_name.endswith(TEXT_SUFFIXES):
